namespace Server.Data
{
	internal static class Data
	{
		public static readonly string[] Quotes = new[]
		{
			"A healthy mind gives a healthy body.",
			"A peaceful mind is a strong mind.",
			"A relaxed mind is a creative mind.",
			"Accept who you are. Focus on who you want to become.",
			"Act selfless, you will be infinite.",
			"Act, don’t react.",
			"Activate your being to be one with all.",
            "All knowledge is within you.",
            "All the manifestation of one light.",
			"Always be pure, simple and honest.",
			"An attitude of gratitude brings you many opportunities.",
			"Appreciate the life you have been given.",
			"Appreciate yourself and honor your soul.",
			"As a plant can’t live without roots, so a human can’t live without a soul.",
			"Ask yourself: Who am I and what am I doing?.",
			"Balance of head and heart yields strength of character.",
			"Be as much outgoing as ingoing, create a balance.",
			"Be aware that as much is above, that much is below.",
			"Be consistent as a constituent part of yourself.",
			"Be fearless knowing that all will be provided at the right time.",
			"Be free. Enjoy yourself.",
			"Be giving, forgiving, compassionate and loving.",
			"Be guided. Listen to the whispers of the universe.",
			"Be great, feel great, act great.",
			"Be great to your own self.",
			"Be happy so long as breath is in you.",
			"Be happy with yourself.",
			"Be interested in your elevation.",
			"Be kind to others and compassionate to yourself.",
			"Be kind to yourself.",
			"Be proud of who you are.",
			"Be so happy that when others look at you they become happy too.",
			"Be you. Be great. Be together.",
			"Be yourself. Everybody else is already taken.",
			"Become part of the universe so the universe becomes part of you.",
			"Befriend your soul.",
			"Belive something higher.",
			"Better to slip with your feet than with your tongue.",
			"Beware of the effects of your actions.",
			"Beware of the power of the spoken word.",
			"Bless every soul, stand by every moment.",
			"Bliss cannot be disturbed by gain or loss.",
			"Bliss is a constant state of mind, undisturbed by gain or loss.",
			"Breathe in peace, breath out blessings to all.",
			"By honoring your words, you are honored in this world.",
			"By honoring your words, you are honored.",
			"By identifying yourself, you are confirming yourself.",
			"By kind and compassionate and the whole world will be your friend.",
			"By listening, you comfort another person.",
			"Celebrate the beauty of unity by loving your life.",
			"Celebrate your success and your failures - they both require great courage.",
			"Chance multiply if you grab them.",
			"Conscious and supreme conscious may meet together, you’ll be happy.",
			"Come together in keeping us all together.",
			"Communicate sacredness, build it, share it and spread it.",
			"Compassion creates understanding.",
			"Compassion is a state of constant giving of the self for others.",
			"Compassion is the fountain of forgiveness.",
			"Compassion is your strength.",
			"Compassion has no limit. Kindness has no enemy.",
			"Compassion will make you beautiful.",
			"Coordination and cooperation is the way of life.",
			"Create the sequence of goodness, consequences will be always good.",
			"Delight the world with kindness, grace and compassion.",
			"Delight the world with your compassion, kindness and grace.",
			"Desirable person is only that, who has no desire.",
			"Develop the power of listening.",
			"Develop your intuition.",
			"Dignity and tranquility last forever.",
			"Doing anything out of compassion, will never be wrong.",
			"Don’t let yourself down, anyone else down or participate in a let down.",
			"Don’t sleep counting sheep, Count blessings, then sleep.",
			"Don’t take pride in taking. Give and you will be given virtues.",
			"Don’t wait for heaven to come to you, create heaven.",
            "Drop your problem and keep going.",
            "Ego limits you.",
			"Empty yourself and let the universe fill you.",
			"Even a journey of thousands miles starts with the first step.",
			"Everlasting impact with compassion and kindness is called love.",
			"Every beat of your heart is a rhythm of your soul.",
			"Every child is a hope for the world.",
			"Every day can be a new beginning.",
			"Every experience is a guidance in life, your response determine the quality.",
			"Every heartbeat creates a miracle.",
			"Every neighbour can be your teacher.",
			"Every promise is a present in abundance.",
			"Every promise is a present in advancement.",
			"Every smile is a direct achievement.",
			"Everybody wants to know whether you are dependable.",
			"Experience is wisdom.",
			"Experience something higher.",
			"Experience the warmth and love of your soul.",
			"Experience will give you the power and confidence to be you.",
			"Experience will give you wisdom.",
			"Experience your own body, your own mind and your own soul.",
			"Feel God within you with each breath.",
			"Feel good, be good and do good.",
			"Feel great, act great and approve of yourself.",
			"Feel great, act great and be great.",
			"Find happiness within yourself. Then share yourself with others.",
			"Find your unity in your service to all.",
			"For every loss there’s an equal gain, for every gain there’s an equal loss.",
			"Forgive every negative thing in your life, forgive it to get out of it.",
			"Forgive your past.",
			"Forgive your past and make room for your future.",
			"Forgiveness is an act of consciousness.",
			"Friendship is a gift that lasts.",
			"Get in touch with yourself.",
			"Give forgiveness, that is your greatness.",
			"Give hapiness and you will end up happy.",
			"Give love, get love.",
			"Give strength to all.",
			"Give to others and give to yourself the same attention.",
			"Give us the strength to resurrect and rise in our excellence.",
			"Give us tranquillity and tolerance, so we can live in compassion.",
			"Goodness should become human nature, because it real in nature.",
			"Grace brings contentment.",
			"Grace brings trust, appreciation, love and prosperity.",
			"Grace is kindness, compassion and caring.",
			"Grace is not measurable by time and space.",
			"Gratitude is the open door to abundance.",
			"Greatness is found in the unity of self.",
			"Greatness is measured by your gifts, not your possessions.",
			"Happiness comes from contentment.",
			"Happiness comes when you overcome the most impossible challenge.",
			"Happiness is actually an art of living, which is within us.",
			"Happiness is an accomplishment.",
			"Happiness is every human being’s birthright.",
			"Happiness is nothing but total relaxation.",
			"Happiness is, when you do things for others.",
			"Have you hugged your soul today?",
			"Have wisdom in your actions and faith in your merits.",
			"Healing comes when triggers dissipate.",
			"Honor projection. Because you have been given a chance.",
			"How can you uplift this moment happening right now?",
			"I am beautiful, I am bountiful, I am blissful.",
			"If we are happy, everybody looks up and shares our happiness.",
			"If you are helping somebody, you are awakening somebody.",
			"If you create a mental duality, you lose your strength.",
			"If you don’t love where you come from, you can’t love where you are going.",
			"If you go out and also go whithin, you’ll create a balance.",
			"If you let yourself be successful, you shall be successful.",
			"If you see good, learn something. If you see bad, learn what not to be.",
			"In every moment of life, you should be what you ought to be.",
			"In life, don’t create just an impression, create a trust.",
			"In order to be remembered, leave nothing behind but goodness.",
			"In order to be remembered, leave nothing behind but love.",
			"In the beginning is you, in the middle is you and in the end is you.",
			"Infinity should always prevail in your life.",
			"Infinity which is your unknown guarantees your happiness.",
			"Inner knowing brings inner peace. Inner peace creates peace in the world.",
			"Inspire through the mercy of your compassionate being.",
			"Inspiration is an unlimited power.",
			"Inspiring others towards happiness brings you happiness.",
			"Intelligence and intuition are two friends.",
			"Impression is for the now, trust is for the future.",
			"It is not talking of love, but living in love that is everything.",
			"It is not what you have that is your greatness; it is what you can give.",
			"It’s essential to purify our bodies and prepare our body, mind and soul.",
			"It’s good to have micro and macro conscience at the same time.",
			"It’s important to find your identity and your legacy.",
			"It’s not life that matters; it’s the courage that we bring to it.",
			"Joy is the essence of success.",
			"Kindness is the essence of life.",
			"Kindness is the gift of life.",
			"Kindness is the light of life.",
			"Kindness is the test of friendship.",
			"Know how to give.",
			"Know that you are chikd of the universe.",
			"Know whatever you are doing is the most beautiful thing.",
			"Knowledge is just a revelation of tomorrow within you.",
			"Knowing yourself is true wisdom, mastering yourself is true power.",
			"Lasting service is the service to uplift all.",
			"Laughing is the same all over the world.",
			"Learn to be noble, courteous and committed.",
			"Learn to identify yourself under every circumstances.",
			"Learn to relate to your soul, the light within you.",
			"Learn to speak the first word to identify yourself.",
			"Let Love elevate your self to excellence.",
			"Let People bask in your radiance and sunshine.",
			"Let things come to you.",
			"Let us be kind and compassionate to remove the sadness of the world.",
			"Let your energy be used to build, not destroy.",
			"Let your greatness show from your inner light.",
			"Let your heart guide you.",
			"Let your heart speak to other’s hearts.",
			"Let your manners speak for you.",
			"Let your mind dance with your body.",
			"Life can be a tricky process, stay in your magic.",
			"Life has meaning.",
			"Life has to be based on our interrelated self-strength.",
			"Life is a chance. Love is infinity. Grace is reality.",
			"Life is a flow of love; your participation is requested.",
			"Life is a gift. Experience is the beauty.",
			"Life is a gift. If you do not value your gift, nobody else will.",
			"Life is life, don’t control it. Allow it.",
			"Life should be consistent with the infinity.",
			"Life without love is a ship without sails.",
			"Lift people up to their potencial and higher self.",
			"Lift up others by your peaceful presence.",
			"Listen and you will develop intuition.",
			"Live and let live.",
			"Live by intuition and consciousness.",
			"Live by your inner knowledge and strength.",
			"Live for each other.",
			"Live from your heart and you will be most effective.",
			"Live in your strength.",
			"Live light, travel light, spread the light, be the light.",
			"Live righteously and love everyone.",
			"Live to love. Love to give.",
			"Live to share.",
			"Live with reverence for yourself and others.",
			"Look for purpose in the place where your deep gladness meets the world’s needs.",
			"Look to that inner strength to walk tall and be happy.",
			"Love, compassion and kindness are the anchors of life.",
			"Love everyone, you will build up an aura of light and love.",
			"Love has no boundary.",
			"Love has no fear and no vengeance.",
			"Love is a source of bliss and infinity.",
			"Love is an elevated self.",
			"Love is an experience of Infinity.",
			"Love is an infinity victory.",
			"Love is ecstasy.",
			"Love is life, life is love.",
			"Love is strength and will give us knowledge.",
			"Love is subject to compassion.",
			"Love is the ultimate law of life.",
			"Love is to live for each other.",
			"Love is to live for somebody, love is not to live with somebody.",
			"Love is unchanging and limitless.",
			"Love is where compassion prevails and kindness rules.",
			"Love the day. Love your life.",
			"Love what is ahead by loving what has come before.",
			"Love whithout trust is a river without water.",
			"Love your soul.",
			"Love yourself so that you may know how to live with self-respect.",
			"Love, compassion and kindness are the anchors of life.",
			"Make yourself so happy that when others look at you they become happy too.",
			"Man is as vast as he acts.",
			"Master the unknown by knowing your deep self.",
			"May all beings be happy.",
			"May the peace of inner self bring us joy.",
			"May this day be the day to lead us to peace, to happiness and to joy.",
			"May this day bring you peace, tranquility and harmony.",
			"May we confine ourselves to the strength of our soul.",
			"May you create peaceful harmony as your life’s purpose.",
			"May you have faith in your worth and act with wisdom.",
			"May you have love, kindness and compassion for all living things.",
			"May you inner self be secure and happy.",
			"May your head and heart speak with one voice.",
			"May your light become a living universal light.",
			"May your mind learn to love with compassion.",
			"May we all be blessed together.",
			"May we confine ourselves to the strength of our soul.",
            "May your head and heart speak with one voice.",
			"Meditation is the medicine of the mind.",
			"Mental happiness is total relaxation.",
			"Mind dwells in the sound and sound dwells in the mind.",
			"Nature is a true giver, a true friend and a sustainer.",
			"Nature teach us harmony.",
			"Never try to impress others, try to impress yourself.",
			"Noble language and behaviors are so powerful that hearts can be melted.",
			"Not sharing is not caring.",
			"Obey, serve, love and excel.",
			"Old age needs wisdom and grace.",
			"One of the best actions we can take with courage is to relax.",
			"One thing never changes, that is infinity.",
			"One who brings happiness to others, will find it.",
			"One who knows how to enjoy life, does not need riches.",
			"Oneness is achieved by recognizing your self.",
			"Oneness within is harmony with all.",
			"Only with self-respect you can respect others.",
			"Only you, yourself can make yourself perfect.",
			"Open up to infinity and you become infinity.",
			"Our creative consciousness is not limited.",
			"Our head bows and our heart is filled with love and joy.",
			"Our highest secret is to identify ourselves openly and honestly.",
			"Our intuition comes from innocence.",
			"Our intuition lies in our innocence.",
			"Our thoughts are forming the world.",
			"Patience gives the power to practice; practice gives the power that leads to perfection.",
			"Patience pays.",
			"Peace is not a place or an outcome. Peace is a pathway to walk on each moment.",
			"Peace of mind comes piece by piece.",
			"Peace within guarantees peace without.",
			"People who love are compassionate.",
			"People who love are forgiving.",
			"People who love are giving.",
			"People who love are happy.",
			"Practice compassion, forgiveness and kindness.",
			"Practice kindness, compassion and caring.",
			"Practice kindness, mercy and forgiveness.",
			"Prosperity is within us.",
			"Provoke, confront, elevate.",
			"Radiate loving is a reflection of your radiant being.",
			"Radiate the infinite light through your finite self.",
			"Real happiness lies in that which never comes nor goes but simply is.",
			"Recieve in gratitude.",
			"Recognize that the other person is you.",
			"Recognize that you are the truth.",
			"Results you achieve by wisdom.",
			"Say it straight, simple and with a smile.",
			"Say what you mean, mean what you say.",
			"See yourself in another, then be kind to yourself.",
			"Seek excellence in every person.",
			"Seek something higher.",
			"Self-reliance conquers any difficulty.",
			"Self-reliance is the greatest art.",
			"Serve all in truth, in compassion, and in grace.",
			"Serve all without classification or discrimination.",
			"Serve humanity so that people feel we are kind to them.",
			"Serve your neighbour with miles of smiles.",
			"Share your inner light and light up all you meet.",
			"Share your strengths, not your weaknesses.",
			"Sing from your heart.",
			"Smiling is the most basic kind of peace work.",
			"Socialise with compassion, kindness and grace.",
			"Socialize with compassion and kindness.",
			"Soul is the highest self.",
			"Speak the truth.",
			"Speak to make yourself happy, don’t speak to impress others.",
			"Spread the light. Be the lighthouse.",
			"Start listening to your inner sound.",
			"Steadiness comes from character and commitment.",
			"Step confidently into the unknown knowing thay you are equal to every moment.",
			"Strength does not lie in what you have. It lies in what you can give.",
			"Only with self-respect you can respect others.",
			"Our head bows and our heart is filled with love and joy.",
			"Our intuition comes from innocence.",
			"Talk to your soul in your imagination.",
			"The art of happiness is to serve all.",
			"The art of longing and the art of belonging must be experienced in life.",
			"The beauty in you is your spirit. The strength in you is your endurance. The intelligence in you is your vastness.",
			"The beauty in you is your spirit.",
			"The beauty of life is to experience yourself.",
			"The beauty of the soul is constant, continuous and endless.",
			"The best way of life is to be, simply be.",
			"The body is a temple: take care of it.",
			"The brightness of your being is generated from within.",
			"The essence of life is to communicate love.",
			"The first word you utter must identify you.",
			"The gate to happiness is self-compassion.",
			"The greatest tool you have is to listen.",
			"The heart sees deeper than the eyes.",
			"The lost is found in the search for self.",
			"The mind is energy: regulate it.",
			"The moment you love, you are unlimited.",
			"The one who listens understands.",
			"The only constant is change.",
			"The only tool you need is kindness.",
			"The power of love is infinite.",
			"The power of prayer extends happiness.",
			"The principle ingredient of life is love.",
			"The purpose of life is to enjoy every moment.",
			"The purpose of life is to live in balance.",
			"The purpose of life is to know, love and trust yourself. Be yourself.",
			"The rhythm of life is when you experience your own body, mind and soul.",
			"The soul is projection: represent it.",
			"The trust that others place in you is your grace.",
			"The truth lies underneath the superficial if you can look for it.",
			"The universe is a stage on which your mind dances with your body, guided by your heart.",
			"The universe is the stage on which you dance, guided by your heart.",
			"The universe is within you. Bright & beautiful.",
			"The unknown has to be trusted.",
			"The unknown is where all outcomes are possible, enter it with grace.",
			"The voice of your soul is breath.",
			"The world needs your unique gifts, don’t leave with them still inside you.",
			"The world wants to see your inner light.",
			"The worst enemy we have is our own fear.",
			"There are three values: Feel good, be good and do good.",
			"There is and there was nothing like you, and there shall be nothing like you.",
			"There is beauty in your presence. Show who you are.",
			"There is no greater power in this universe than the power of prayer.",
			"There is no greater power than the power of the word.",
			"There is no knowledge outside. All knowledge is within you.",
			"There is no love without compassion.",
			"There is no more decoration and value than your own kindness.",
			"There is nothing more precious than the self.",
			"There is nothing more precious than self-trust.",
			"Think seriously and think honestly.",
			"This body needs to be understood to be taken care of.",
			"This life is a gift.",
			"Those who are selfless find God.",
			"Those who have found guidance are blessed.",
			"Those who have found their fulfillment are blessed.",
			"Those who have found their soul and their self are blessed.",
			"Those who live in the past limit their future.",
			"To be calm is the highest achievement of the self.",
			"To be great, feel great and act great.",
			"To be healthy: eat right, walk right and talk to yourself right.",
			"To know others is smart. To know yourself is wise.",
			"To learn, read. To know, write. To master, teach.",
			"Together we can do what we can never do alone.",
			"Tomorrow has to become today to deal with it, you don’t have to bother.",
			"Totally control the concentration.",
			"Tranquility is the essence of life.",
			"Travel light, live light, spread the light, be the light.",
			"True understanding is found through compassion.",
			"True wealth is the ability to let go of your possessions.",
			"Trust creates peace.",
			"Trust is the question. Love is the solution.",
			"Trust is the union of intelligence and integrity.",
			"Trust the wisdom of the heart.",
			"Trust your identity be in touch with your reality.",
			"Truth is everlasting.",
			"Uncage your heart, free your heart, let it be wild.",
			"Understand life is a pure living experience.",
			"Understand what is going to be.",
			"Understanding is found through compassion.",
			"Unfold your own myth.",
			"Unite with your own higher self and create a friendship.",
			"Uplift everybody and uplift yourself.",
			"Use your head to live with heart.",
			"Walk beautifully, talk beautifully, live beautifully.",
			"We all know we have a destiny.",
			"We all want to know the truth.",
			"We are born wise, we are born complete.",
			"We are here to love each other, serve each other and uplift each other.",
			"We are spiritual beings having a human experience.",
			"We can always start again.",
			"We must have a meditative power to understand time.",
			"We pay more attention to gossip than to gospels.",
			"We should be in a position to intuitively feel.",
			"We should be strong and true.",
			"We should learn how to be graceful and honorable.",
			"What belongs to you shall come to you.",
			"What can you appreciate in this moment?.",
			"Whatever character you give your children shall be their future.",
			"Whatever you are doing is the most beautiful thing.",
			"Whatever you are facing, just might become your greatest gift.",
			"Whatever you are, you are. Be proud of it.",
			"When ego is lost, limit is lost.",
			"When ego is lost, then universality exists.",
			"When fear is forcing you to give up, call on your heart’s courage to continue.",
			"When people glow and grow, life becomes wery happy.",
			"When the dawn of simplicity comes in life, complications leave.",
			"When the ego is lost, limit is lost. You become infinite, kind, beautiful.",
			"When the mind is backed by will, miracles happen.",
			"When we practice listening, we become intuitive.",
			"When you act with compassion you will never be wrong.",
			"When you are in tune with the unknown, the known is peaceful.",
			"When you identify yourself a lot of good things happen.",
			"When you know that all is light, you are enlightened.",
			"When you refuse to become upset, you are also refusing to be set up.",
			"When your inner and outer worlds harmonize, your relationships do too.",
			"Where there is love, there is no question.",
			"Why not enjoy today? It will be gone tomorrow.",
			"Wisdom becomes knowledge when it is a personal experience.",
			"Wisdom is listening, foolishness is telling.",
			"Wisdom, character and consciousness conquer everything.",
			"Without gratitude, there is no prosperity.",
			"Without realizing who you are, happiness cannot come to you.",
			"Without sacrifice there is no happiness.",
			"Without the darkness, you would never know the light.",
			"Work, but do not forget to live.",
			"You are a living consciousness.",
			"You are equally as beautiful as the universe.",
			"You are infinite.",
			"You are remembered for your goodness.",
			"You are the composer of your life. To hear the music, reduce the noise.",
			"You are the waster of your judgment, decision and action.",
			"You are the way you are, that is now.",
			"You are unlimited.",
			"You can run after satisfaction, but satisfaction must come from within.",
			"You don’t need love, you are the love.",
			"You great happiness is based on the warmth of your heart.",
			"You have not to find out the reality, you are the reality.",
			"You have the wings of grace.",
			"You head must bow to your heart.",
			"You live forever in the memory of the love you have shared.",
			"You must know that you can swim through every tide and change of time.",
			"You must live for something higher, bigger and better than you.",
			"You need leadership for yourself within yourself.",
			"You only give when you love.",
			"You should develop mental strength.",
			"You travel in grace knowing you are protected by love.",
			"You will always live happy if you live with heart.",
			"You will feel fulfilled when you do the impossible for someone else.",
			"Your base should be accurate and your projection should be accurate.",
			"Your breath is the voice of your soul.",
			"Your breath fills you with energy anywhere and anytime you need it.",
			"Your deep inner self is your brightest light.",
			"Your destiny is to merge with infinity.",
			"Your entire value is your word.",
			"Your great happiness is based on the warmth of your heart.",
			"Your greatest strength is love.",
			"Your greatness is measured by your gifts, not by what you have.",
			"Your greatness is not what you have; it’s what you give.",
			"Your heartbeat is the rhythm of your soul.",
			"Your indentifying nature is your real nature.",
			"Your infinity in you is the reality in you.",
			"Your inner self is your inner guide.",
			"Your intuition is your best friend.",
			"Your life is based on the capacity of energy in you not outside of you.",
			"Your manifestation is your reality.",
			"Your mind is the flow of God.",
			"Your personality is how radiant you are.",
			"Your potential self is infinite.",
			"Your respect and reverence depends upon your first utterance of the world.",
			"Your soul is your highest self.",
			"Your strength is in how calmly, quietly and peacefully you face life.",
			"Your strength is your own belief.",
			"Your strength is your own knowledge.",
			"Your teacher is your own intuition.",
			"Your true nature is trust.",
			"Your values are magnetically attracting things.",
			"Your word is your greatest power.",
		};
	}
}